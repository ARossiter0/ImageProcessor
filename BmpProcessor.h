...
struct BMP_Header {
    ...
    short reserved1;        //App. specific
    short reserved2;        //App. specific
    int offset_pixel_array; //Offset where pixel array can be found
};

struct DIB_Header{
    int header_size;        //# of bytes in DIB header
    int width;              //Width of the bitmap it pixels
    int height;             //Height of the bitmap it pixels
    short planes;           //Number of color planes used
    short bits_per_pixel;   //Number of bits per pixel
    int compression;        //Array compression used
    int image_size;         //Size of raw bitmap data(includes padding)
    int x_pxl_per_meter;    //print resolution
    int y_pxl_per_meter;    ///////////////////////////////////////////////////
    int colors_in_table;    //Number of colors in palette
    int imp_color_count;    //Important colors(0 means all are important)
};
