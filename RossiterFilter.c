/**
* Program that takes in 3 arguments,
* -i input file name
* -o output file name
* -f filter to be used: b - blur, c - swiss cheese
* Copies input file and applies a filter to it and saves it into a new file.
*
* Completion time: 6 Hours
*
* @author Alexander Rossiter, Acuna
* @version 1.0
*/
...
void* blurPixels(void* param) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = section * size; j < (section + 1) * size; j++) {
            redTotal = in_pArr[i][j].r;
            greenTotal = in_pArr[i][j].g;
            blueTotal = in_pArr[i][j].b;
            num_pix = 1;
            if (i != 0 && j != 0) {
                redTotal += in_pArr[i-1][j-1].r; //top left pixel
                greenTotal += in_pArr[i-1][j-1].g;
                blueTotal += in_pArr[i-1][j-1].b;
                num_pix++;
            }
            if (i != 0) {
                redTotal += in_pArr[i-1][ j ].r; //top pixel
                greenTotal += in_pArr[i-1][ j ].g;
                blueTotal += in_pArr[i-1][ j ].b;
                num_pix++;
            }
            if (i != 0 && j != width - 1) {
                redTotal += in_pArr[i-1][j+1].r; //top right pixel
                greenTotal += in_pArr[i-1][j+1].g;
                blueTotal += in_pArr[i-1][j+1].b;
                num_pix++;
            }
            if (j != 0) {
                redTotal += in_pArr[ i ][j-1].r; //left pixel
                greenTotal += in_pArr[ i ][j-1].g;
                blueTotal += in_pArr[ i ][j-1].b;
                num_pix++;
            }
            if (j != width - 1) {
                redTotal += in_pArr[ i ][j+1].r; //right pixel
                greenTotal += in_pArr[ i ][j+1].g;
                blueTotal += in_pArr[ i ][j+1].b;
                num_pix++;
            }
            if (i != height - 1 && j != 0) {
                redTotal += in_pArr[i+1][j-1].r; //bottom left pixel
                greenTotal += in_pArr[i+1][j-1].g;
                blueTotal += in_pArr[i+1][j-1].b;
                num_pix++;
            }
            if (i != height - 1) {
                redTotal += in_pArr[i+1][ j ].r; //bottom pixel
                greenTotal += in_pArr[i+1][ j ].g;
                blueTotal += in_pArr[i+1][ j ].b;
                num_pix++;
            }
            if (i != height - 1 && j != width - 1) {
                redTotal += in_pArr[i+1][j+1].r; //bottom right pixel
                greenTotal += in_pArr[i+1][j+1].g;
                blueTotal += in_pArr[i+1][j+1].b;
                num_pix++;
            }
            out_pArr[i][j].r = redTotal / num_pix;
            out_pArr[i][j].g = greenTotal / num_pix;
            out_pArr[i][j].b = blueTotal / num_pix;
        }
    }
